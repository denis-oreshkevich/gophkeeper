...
// Process является удобной оберткой над exec.Cmd
...
// NewProcess returns new unstarted background process instance.
func NewProcess(ctx context.Context, command string, opts ...ProcessOpt) *Process {
	p := &Process{
...
func (p *Process) WaitPort(ctx context.Context, network, port string) error {
...
func (p *Process) ListenPort(ctx context.Context, network, port string) error {
	ticker := time.NewTicker(p.waitPortInterval)
	defer ticker.Stop()
